...
### Added
- 計算処理 `Compute()` 実行時に現在の計算モードやエージェント選択、各パラメータ等を `LocalStorage` へ保存する機能を実装
- `LocalStorage` に保存された内容をページ読み込み時に反映させる機能を実装

### Changed
- 初期化ボタンを押下した時に `<Select>`, `<checkbox>`要素もリセットするように変更