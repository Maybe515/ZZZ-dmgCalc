...
import { defaults, selectDefaults, anomalyCorrTable, rangeTable, matchTable, imgPaths, urls, attributeValueMap } from "./data.js";
...
function updateAttrGroup(attrs, prefix, dict) {
  for (let i = 0; i < 2; i++) {
    ...
    updateIcon(baseId, attrId, `${imgPaths.base}${imgPaths.attribute}`, t(dict, "ui.attributeLabel", "Attribute"));
  }
}
...
function updateAgentInfo(dict = {}) {
  ...
  updateIcon("faction", agent.factionId, `${imgPaths.base}${imgPaths.faction}`, t(dict, "ui.factionLabel", "Faction"));
  updateIcon("specialty", agent.specialtyId, `${imgPaths.base}${imgPaths.specialty}`, t(dict, "ui.specialtyLabel", "Specialty"));
  updateIcon("attribute", agent.attributeId, `${imgPaths.base}${imgPaths.attribute}`, t(dict, "ui.attributeLabel", "Attribute"));

  $("attrSelect").value = attributeValueMap[agent.attributeId] ?? "";
  updateAnomalyCorr();

  updateImage("agentImage", agent.image ? `${imgPaths.base}${imgPaths.agent}${agent.image}` : "", agent.image ? `画像:${t(dict, `agent.${sel}`, sel)}` : "");
  updateImage("rankImage", agent.rank ? `${imgPaths.base}${imgPaths.rank}rank_${agent.rank}.png` : "", agent.rank ? `ランク${agent.rank}` : "");
...
function updateEnemyInfo(dict = {}) {
  ...
  updateImage("enemyImage", enemy.image ? `${imgPaths.base}${imgPaths.enemy}${enemy.image}` : "", enemy.image ? `画像:${t(dict, `enemy.${sel}`, sel)}` : "");
...
async function loadAllData() {
  return Promise.all([
    loadJSON("./assets/data/agents.json", agents, () => populateSelect("agentSelect", agents, i18nDict)),
    loadJSON("./assets/data/enemies.json", enemies, () => populateSelect("enemySelect", enemies, i18nDict)),
    loadJSON("./assets/data/lvCoeffTable.json", lvCoeffTable),
    loadJSON("./assets/data/helpTexts.json", helpTexts),
    loadJSON(`./assets/data/lang/${$("langSelect").value || "jp"}.json`, i18nDict, dict => applyLanguage(dict))
  ]);
}
...
async function loadLanguage() {
  const lang = $("langSelect")?.value || "jp";
  await loadJSON(`./assets/data/lang/${$("langSelect").value || "jp"}.json`, i18nDict, dict => applyLanguage(dict));
}
...
document.querySelectorAll("details").forEach(details => {
  ...
  summary.addEventListener("click", e => {
    ...
    // 閉じるときの処理
    e.preventDefault(); // デフォルトの即閉じを止める
